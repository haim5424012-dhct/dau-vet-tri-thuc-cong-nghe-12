...
const TEACHER_PROFILE = {
  name: "Võ Văn Bé Hai",
  role: "Giáo viên Công nghệ 12 · Tổ chuyên môn Sinh học",
  school: "Trường Trung học phổ thông Đốc Binh Kiều – Cai Lậy",
  motto: "Học bằng tò mò. Quyết định bằng bằng chứng.",
};

const SAMPLE_REPORT_LINK = "https://docs.google.com/document/d/ID-MAU-THAY-BANG-ID-THAT/edit?usp=sharing";
const DRIVE_SHARING_HELP_URL = "https://support.google.com/drive/answer/2494822?hl=vi&co=GENIE.Platform%3DDesktop";
...
type IndustryKind = "forest" | "water";
const getReportKind = (code: string): IndustryKind => groups.find((group) => group.reports.some((report) => report.code === code))?.kind as IndustryKind ?? "forest";
...
type FieldErrorKey = keyof FormState | "submit";
type FieldErrors = Partial<Record<FieldErrorKey, string>>;
const normalizeCodeInput = (value: string) => value.trim().toUpperCase().replace(/\s+/g, "");
const validateSubmissionForm = (value: FormState, selectedGroup: typeof groups[number], answeredCount: number): FieldErrors => {
  const errors: FieldErrors = {};
  if (!value.student.trim()) errors.student = "Nhập họ và tên để giáo viên nhận diện bài nộp.";
  if (!value.className.trim()) errors.className = "Nhập lớp, ví dụ 12A1.";
  if (!value.classCode.trim()) errors.classCode = "Nhập mã lớp, ví dụ 12A1.";
  if (!/^N[1-8]$/.test(value.groupCode)) errors.groupCode = "Mã nhóm phải có dạng N1 đến N8, ví dụ N3.";
  if (!selectedGroup.reports.some((report) => report.code === value.reportCode)) errors.reportCode = `Mã báo cáo không thuộc ${selectedGroup.id}. Hãy chọn một trong hai mã đang hiển thị.`;
  if (!value.reportTitle.trim()) errors.reportTitle = "Đặt tên ngắn gọn cho sản phẩm nhóm đã thực hiện.";
  if (!value.reportLink.trim()) errors.reportLink = "Dán link Google Drive hoặc Google Slides của báo cáo.";
  else if (!isSafeReportLink(value.reportLink)) errors.reportLink = "Link phải bắt đầu bằng https:// và thuộc docs.google.com, drive.google.com hoặc googleusercontent.com.";
  if (answeredCount < criteria.length) errors.submit = "Hãy chọn đủ 5 tiêu chí rubric trước khi gửi.";
  return errors;
};
...
export default function Home() {
  ...
  const [sampleLinkInUse, setSampleLinkInUse] = useState(false);
  ...
  const updateForm = (key: keyof typeof form, value: string) => {
    const normalizedValue = ["classCode", "groupCode", "reportCode"].includes(key) ? normalizeCodeInput(value) : value;
    setForm((current) => ({ ...current, [key]: normalizedValue }));
    if (key === "reportLink") setSampleLinkInUse(false);
    setFieldErrors((current) => ({ ...current, [key]: undefined, submit: undefined }));
  };
  const pasteSampleLink = () => {
    updateForm("reportLink", SAMPLE_REPORT_LINK);
    setSampleLinkInUse(true);
    setFieldErrors((current) => ({ ...current, reportLink: "Đây là link minh họa. Hãy thay bằng link báo cáo thật trước khi gửi." }));
    toast.info("Đã điền link mẫu. Hãy thay ID-MAU-THAY-BANG-ID-THAT bằng link thật của nhóm.");
  };
  const checkViewPermission = () => {
    if (sampleLinkInUse) {
      setFieldErrors((current) => ({ ...current, reportLink: "Bạn đang dùng link mẫu. Hãy thay bằng link báo cáo thật trước khi kiểm tra." }));
      toast.error("Không thể kiểm tra link mẫu. Hãy dán link báo cáo thật trước.");
      return;
    }
    if (!isSafeReportLink(form.reportLink)) {
      setFieldErrors((current) => ({ ...current, reportLink: "Hãy nhập link HTTPS của Google Drive/Slides trước khi kiểm tra." }));
      window.setTimeout(() => document.getElementById("field-reportLink")?.focus(), 0);
      toast.error("Link chưa đúng định dạng Google Drive/Slides.");
      return;
    }
    const opened = window.open(form.reportLink, "_blank", "noopener,noreferrer");
    if (opened) toast.info("Đã mở báo cáo ở tab mới. Hãy kiểm tra khi chưa đăng nhập hoặc bằng cửa sổ ẩn danh.");
    else toast.error("Trình duyệt đã chặn tab mới. Hãy cho phép cửa sổ bật lên rồi thử lại.");
  };
  const validateField = (key: FieldErrorKey) => {
    const errors = validateSubmissionForm(form, selected, answered);
    if (key === "reportLink" && sampleLinkInUse) errors.reportLink = "Bạn đang dùng link mẫu. Hãy thay bằng link Google Drive/Slides thật.";
    setFieldErrors((current) => ({ ...current, [key]: errors[key] }));
  };
  const validateAndFocus = () => {
    const errors = validateSubmissionForm(form, selected, answered);
    setFieldErrors(errors);
    const firstError = Object.keys(errors).find((key) => key !== "submit");
    if (firstError) window.setTimeout(() => document.getElementById(`field-${firstError}`)?.focus(), 0);
    return errors;
  };
  ...
  const submit = async () => {
    ...
    if (Object.keys(errors).length) {
      toast.error("Chưa thể gửi: hãy sửa các mục được đánh dấu trong biểu mẫu."); return;
    }
  ...
  return <div className="site-shell min-h-screen bg-[#f5f1e8] text-[#17352d]">
    <header className="site-header sticky top-0 z-30 border-b border-[#d9d3c6] bg-[#f5f1e8]/95 backdrop-blur-md">
      ...
      <div className="teacher-strip border-t border-[#e3ded2] bg-[#f0ece2]/75 px-5 py-3 lg:px-10">
    ...
    <main id="top"><aside className="journal-rail" aria-label="Mục lục sổ tay"><span className="journal-rail-mark">INDEX / 01</span><a href="#quy-trinh">Quy trình</a><a href="#chu-de">YCCĐ · NLS</a><a href="#rubric">Rubric</a><a href="#gui-bai">Nộp bài</a><a href="#kiem-chung">Kiểm chứng AI</a><span className="journal-rail-line" /></aside>
      <section className="hero-section relative overflow-hidden border-b border-[#d9d3c6]">
        <div className="mx-auto grid max-w-[1440px] items-end gap-10 px-5 pb-16 pt-14 lg:grid-cols-[0.92fr_1.08fr] lg:px-10 lg:pb-24 lg:pt-20">
          <div className="relative z-10 max-w-xl"><div className="hero-kicker mb-6 inline-flex items-center gap-2 rounded-full border border-[#a9c4b5] bg-[#e4efe8] px-3 py-1.5 text-xs font-bold uppercase tracking-[0.17em] text-[#1f6b57]"><Sprout size={14} /> Nhật ký học tập số</div><h1 className="hero-title font-serif text-5xl leading-[0.98] tracking-[-0.04em] text-[#17352d] md:text-7xl">Biến báo cáo thành <em className="hero-accent text-[#1f6b57]">dấu vết tri thức.</em></h1><p className="mt-7 max-w-md text-lg leading-8 text-[#53675f]">Một không gian nộp báo cáo, khám phá công nghệ và tự phản tư dành cho 8 nhóm chủ đề Lâm nghiệp – Thủy sản.</p><div className="mt-9 flex flex-wrap gap-3"><a href="#rubric"><Button className="h-12 rounded-full bg-[#1f6b57] px-6 text-white hover:bg-[#174e40]">Bắt đầu tự chấm <ArrowRight className="ml-2" size={17} /></Button></a><a href="#quy-trinh"><Button variant="outline" className="h-12 rounded-full border-[#a9b8ad] bg-transparent px-6">Xem quy trình</Button></a></div></div>
          <div className="hero-visual relative min-h-[310px] overflow-hidden rounded-[2rem] border border-[#c8d8cc] bg-[#dcebe1] shadow-[0_18px_50px_rgba(31,107,87,0.12)]"><img src={`${ASSET_BASE}/fieldnote-hero.jpg`} alt="Minh họa hệ sinh thái rừng và thủy sản" className="absolute inset-0 h-full w-full object-cover mix-blend-multiply opacity-80" /><div className="absolute bottom-5 left-5 max-w-xs rounded-2xl border border-white/50 bg-[#f5f1e8]/90 p-4 backdrop-blur-sm"><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.16em] text-[#1f6b57]"><Leaf size={14} /> Field note 01</div><p className="mt-2 font-serif text-xl leading-tight">“AI cảnh báo — con người kiểm tra và quyết định.”</p></div></div>
        </div>
      </section>
      ...
      <section id="chu-de" className="border-y border-[#d9d3c6] bg-[#e8efe9] px-5 py-16 lg:px-10 lg:py-24"><div className="mx-auto max-w-[1440px]"><div className="max-w-2xl"><p className="eyebrow">Bản đồ nội dung</p><h2 className="section-title">Chọn đúng nhóm, mở đúng yêu cầu.</h2><p className="mt-4 text-[#5b7067]">Mỗi thẻ dưới đây được ánh xạ từ PL1/PL3, gồm cụm nội dung, bài học, mã Khung 3439 và yêu cầu cần đưa vào báo cáo. Bài 1–2 “Giới thiệu chung về lâm nghiệp” không còn là nhóm lựa chọn trong phiên bản này.</p></div><div className="mb-5 flex flex-wrap gap-3" aria-label="Chú giải cụm ngành"><span className="industry-chip industry-chip-forest"><Leaf size={14} aria-hidden="true" /> Lâm nghiệp · N1–N4</span><span className="industry-chip industry-chip-water"><Waves size={14} aria-hidden="true" /> Thủy sản · N5–N8</span></div><div className="mt-10 grid gap-4 lg:grid-cols-[0.85fr_1.15fr]"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">{groups.map((group) => <button key={group.id} onClick={() => setActiveGroup(group.id)} className={`group-card group-card-${group.kind} flex items-center justify-between rounded-2xl border p-4 text-left transition-all duration-200 ${activeGroup === group.id ? "group-card-active text-white shadow-lg" : ""}`}><span><span className={`group-card-label block text-xs font-bold uppercase tracking-[0.14em] ${activeGroup === group.id ? "text-white/80" : ""}`}>{group.id} · {group.chapter}</span><span className="mt-1 block font-serif text-lg">{group.title}</span></span><ArrowRight size={18} /></button>)}</div><Card className={`topic-detail-card topic-detail-${selected.kind} overflow-hidden shadow-none`}><div className="grid md:grid-cols-[0.75fr_1.25fr]"><div className={`relative min-h-[260px] overflow-hidden topic-image-${selected.kind}`}><img src={selected.kind === "forest" ? `${ASSET_BASE}/fieldnote-forest.jpg` : `${ASSET_BASE}/fieldnote-aquaculture.jpg`} alt="Minh họa chủ đề" className="h-full w-full object-cover mix-blend-multiply opacity-85" /><div className="absolute left-5 top-5 flex flex-wrap gap-2">{selected.codes.map((code) => <Badge key={code} className={`topic-code topic-code-${selected.kind}`}>{code}</Badge>)}</div></div><CardContent className="p-7 lg:p-10"><div className="flex items-center justify-between"><span className="eyebrow">{selected.lessons}</span><span className={`industry-chip industry-chip-${selected.kind}`}><span className="sr-only">Cụm ngành: </span>{selected.kind === "forest" ? <Leaf size={14} aria-hidden="true" /> : <Waves size={14} aria-hidden="true" />}{selected.kind === "forest" ? "Lâm nghiệp" : "Thủy sản"} · {selected.id}</span></div><h3 className="mt-4 font-serif text-3xl leading-tight">{selected.title}</h3><div className="mt-7 border-l-2 border-[#a9c4b5] pl-4"><p className="text-sm font-semibold uppercase tracking-[0.12em] text-[#71857a]">Điểm cần có trong báo cáo</p><p className="mt-2 leading-7 text-[#4c6258]">{selected.outcome}</p></div><div className="mt-7 grid gap-3 sm:grid-cols-2"><div className="sm:col-span-2"><p className="text-sm font-semibold uppercase tracking-[0.12em] text-[#71857a]">Hai báo cáo trong học kỳ</p><p className="mt-1 text-xs text-[#74857b]">Chọn đúng mã khi nộp để giáo viên theo dõi tiến độ từng nhóm.</p></div>{selected.reports.map((report) => <article key={report.code} className={`report-plan-card report-plan-${selected.kind} rounded-xl border p-4`}><div className="flex items-start justify-between gap-2"><span className={`report-code-chip report-code-chip-${selected.kind}`}>{selected.kind === "forest" ? <Leaf size={13} aria-hidden="true" /> : <Waves size={13} aria-hidden="true" />}{report.code}</span><span className="text-xs text-[#83938a]">{report.scope}</span></div><h4 className="mt-2 font-semibold text-[#315d4d]">{report.title}</h4><p className="mt-1 text-xs leading-5 text-[#63766c]">{report.deliverable}</p></article>)}</div><a href="#rubric" className="mt-8 inline-flex items-center text-sm font-bold text-[#1f6b57]">Đến phần tự chấm <ArrowDown size={16} className="ml-2" /></a></CardContent></div></Card></div></div></section>
