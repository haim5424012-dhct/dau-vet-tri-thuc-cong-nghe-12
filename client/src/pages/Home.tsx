...
const groups = [
  { id: "N1", chapter: "Chương I", title: "Giới thiệu chung về lâm nghiệp", lessons: "Bài 1–2", codes: ["12.C2", "12.A1"], kind: "forest", outcome: "Nêu một ứng dụng có thể hỗ trợ phân tích ảnh vệ tinh hoặc nhận diện dấu hiệu bất thường; mô tả dữ liệu đầu vào, cảnh báo đầu ra và phân biệt rõ cảnh báo của hệ thống với quyết định của kiểm lâm." },
  { id: "N2", chapter: "Chương II", title: "Trồng và chăm sóc rừng", lessons: "Bài 3–5", codes: ["12.C2"], kind: "forest", outcome: "Giải thích cách ảnh vệ tinh/drone có thể hỗ trợ theo dõi độ che phủ hoặc số cây; không khẳng định hệ thống tự quyết định thời vụ nếu chưa có dữ liệu, mô hình và kiểm chứng phù hợp." },
  { id: "N3", chapter: "Chương III", title: "Bảo vệ và khai thác rừng bền vững", lessons: "Bài 6–7", codes: ["12.C2", "12.A1"], kind: "forest", outcome: "Mô tả cách ảnh/camera hoặc mô hình phân tích có thể phát hiện dấu hiệu suy giảm, cháy rừng hoặc chặt phá; nêu sai số, bước kiểm tra và vai trò quyết định của con người." },
  { id: "N4", chapter: "Chương IV + V", title: "Thuỷ sản và môi trường nuôi", lessons: "Bài 8–12", codes: ["12.C2", "12.A1"], kind: "water", outcome: "Phân biệt cảm biến chỉ đo với phần mềm phân tích hoặc mô hình dự báo pH, oxygen, nhiệt độ; giải thích rằng cảnh báo cần ngưỡng, dữ liệu đủ tin cậy và con người quyết định xử lí." },
  { id: "N5", chapter: "Chương VI", title: "Công nghệ giống thuỷ sản", lessons: "Bài 13–15", codes: ["12.C2"], kind: "water", outcome: "Trình bày YCCĐ về vai trò giống và sinh sản; nếu liên hệ AI, chỉ xem là mở rộng về phân tích kích thước/hình dạng, không coi đó là yêu cầu bắt buộc vì Bài 13–14 không có mã AI riêng trong PL1." },
  { id: "N6", chapter: "Chương VII", title: "Công nghệ thức ăn nuôi thuỷ sản", lessons: "Bài 16–18", codes: ["12.C2"], kind: "water", outcome: "Giải thích ở mức minh họa cách camera và phần mềm có thể phân tích hành vi bắt mồi để hỗ trợ điều chỉnh thức ăn; cần nêu dữ liệu, sai số và việc người nuôi kiểm tra trước khi áp dụng." },
  { id: "N7", chapter: "Chương VIII", title: "Công nghệ nuôi thuỷ sản", lessons: "Bài 19–22", codes: ["12.C2", "12.A3"], kind: "water", outcome: "Phân tích vai trò của cảm biến/camera và phần mềm trong cảnh báo sớm; trình bày trách nhiệm ghi nhật kí số trung thực, truy xuất được và không sửa dữ liệu để làm đẹp kết quả theo tinh thần VietGAP." },
  { id: "N8", chapter: "Chương IX + X", title: "Bệnh và nguồn lợi thuỷ sản", lessons: "Bài 23–27", codes: ["12.C2"], kind: "water", outcome: "Nêu một ví dụ nghiên cứu/ứng dụng về phân tích ảnh hoặc dữ liệu để hỗ trợ phát hiện dấu hiệu bệnh, giám sát khu bảo tồn hay dự báo ngư trường; không chẩn đoán bệnh hoặc quyết định khai thác chỉ dựa vào AI." },
];
...
const aiEvidence = [
  ...
  { title: "Không ép mọi nhóm phải có cùng một sản phẩm AI", text: "N5 (Bài 13–14) và N6 (Bài 17–18) không có mã AI riêng trong bảng nguồn PL1. Các nhóm này có thể liên hệ mở rộng, nhưng vẫn phải đạt YCCĐ Công nghệ 12 trước khi nói về AI.", tag: "Phân hóa nhiệm vụ" },
];
...
export default function Home() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const record: SubmissionRecord = { ...form, group: selected.id, groupTitle: selected.title, score: Math.round(score), submittedAt: new Date().toISOString() };
  ...
  const filteredSubmissions = submissions.filter((item) => (statsClass === "all" || item.classCode === statsClass) && (statsGroup === "all" || item.groupCode === statsGroup));
  ...
  const exportRows = filteredSubmissions.map((item) => ({
    ...
    "Nhóm chủ đề": `${item.group} — ${item.groupTitle}`, "Tên báo cáo": item.reportTitle, "Link báo cáo": item.reportLink,
  ...
  const downloadExcel = () => {
    ...
    worksheet["!cols"] = [{ wch: 12 }, { wch: 12 }, { wch: 22 }, { wch: 10 }, { wch: 38 }, { wch: 42 }, { wch: 58 }, { wch: 16 }, { wch: 22 }];
  ...
  return <div className="min-h-screen bg-[#f5f1e8] text-[#17352d]">
    ...
    <main id="top"><aside className="journal-rail" aria-label="Mục lục sổ tay"><span className="journal-rail-mark">INDEX / 01</span><a href="#quy-trinh">Quy trình</a><a href="#chu-de">YCCĐ · NLS</a><a href="#rubric">Rubric</a><a href="#gui-bai">Nộp bài</a><a href="#kiem-chung">Kiểm chứng AI</a><span className="journal-rail-line" /></aside>
      ...
      <section id="thong-ke" className="border-t border-[#d9d3c6] bg-[#e8efe9] px-5 py-16 lg:px-10 lg:py-24"><div className="mx-auto max-w-[1440px]"><div className="flex flex-wrap items-end justify-between gap-6"><div><p className="eyebrow">Evidence ledger / local view</p><h2 className="section-title">Thống kê theo mã lớp và mã nhóm.</h2><p className="mt-4 max-w-2xl leading-7 text-[#5b7067]">Bảng này tổng hợp các bản ghi đã gửi từ trình duyệt hiện tại. Google Sheet vẫn là nguồn dữ liệu chính thức để giáo viên đối chiếu toàn lớp.</p></div><BarChart3 className="text-[#1f6b57]" size={38} /></div><div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-2xl border border-[#cbd8ce] bg-[#f5f1e8] p-5"><span className="ledger-tag">records</span><strong className="mt-3 block font-serif text-4xl">{filteredSubmissions.length}</strong><span className="text-sm text-[#6c7c73]">bản ghi đang lọc</span></div><div className="rounded-2xl border border-[#cbd8ce] bg-[#f5f1e8] p-5"><span className="ledger-tag">average</span><strong className="mt-3 block font-serif text-4xl">{averageScore}<small className="ml-1 font-mono text-base">/100</small></strong><span className="text-sm text-[#6c7c73]">điểm trung bình</span></div><div className="rounded-2xl border border-[#cbd8ce] bg-[#f5f1e8] p-5"><span className="ledger-tag">highest</span><strong className="mt-3 block font-serif text-4xl">{highestScore}<small className="ml-1 font-mono text-base">/100</small></strong><span className="text-sm text-[#6c7c73]">điểm cao nhất</span></div><div className="rounded-2xl border border-[#cbd8ce] bg-[#f5f1e8] p-5"><span className="ledger-tag">source</span><strong className="mt-3 block font-serif text-2xl">Trình duyệt</strong><span className="text-sm text-[#6c7c73]">không thay thế Google Sheet</span></div></div><div className="mt-8 flex flex-wrap gap-3"><select value={statsClass} onChange={(e) => setStatsClass(e.target.value)} className="h-11 rounded-full border border-[#b9c8bc] bg-[#f5f1e8] px-4 text-sm font-semibold text-[#17352d]"><option value="all">Tất cả mã lớp</option>{classOptions.map((value) => <option key={value} value={value}>{value}</option>)}</select><select value={statsGroup} onChange={(e) => setStatsGroup(e.target.value)} className="h-11 rounded-full border border-[#b9c8bc] bg-[#f5f1e8] px-4 text-sm font-semibold text-[#17352d]"><option value="all">Tất cả mã nhóm</option>{groupOptions.map((value) => <option key={value} value={value}>{value}</option>)}</select><button type="button" onClick={() => { setStatsClass("all"); setStatsGroup("all"); }} className="inline-flex h-11 items-center gap-2 rounded-full border border-[#b9c8bc] px-4 text-sm font-semibold text-[#1f6b57]"><RefreshCw size={15} />Đặt lại bộ lọc</button><button type="button" onClick={downloadCsv} className="inline-flex h-11 items-center gap-2 rounded-full bg-[#1f6b57] px-4 text-sm font-semibold text-white shadow-sm"><Download size={15} />CSV</button><button type="button" onClick={downloadExcel} className="inline-flex h-11 items-center gap-2 rounded-full bg-[#e7bc65] px-4 text-sm font-semibold text-[#17352d] shadow-sm"><Download size={15} />Excel</button></div><div className="mt-5 overflow-x-auto rounded-2xl border border-[#cbd8ce] bg-[#f5f1e8]"><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-b border-[#d9d3c6] text-xs uppercase tracking-[0.12em] text-[#728279]"><tr><th className="px-5 py-4">Mã lớp</th><th className="px-5 py-4">Mã nhóm</th><th className="px-5 py-4">Tên học sinh</th><th className="px-5 py-4">Báo cáo</th><th className="px-5 py-4">Điểm</th><th className="px-5 py-4">Thời gian</th></tr></thead><tbody>{filteredSubmissions.length ? filteredSubmissions.map((item) => <tr key={`${item.submittedAt}-${item.student}`} className="border-b border-[#e4dfd4] last:border-0"><td className="px-5 py-4 font-mono font-semibold text-[#1f6b57]">{item.classCode}</td><td className="px-5 py-4 font-mono">{item.groupCode}</td><td className="px-5 py-4">{item.student}</td><td className="max-w-[280px] truncate px-5 py-4">{item.reportTitle}</td><td className="px-5 py-4 font-mono font-bold">{item.score}/100</td><td className="px-5 py-4 text-xs text-[#718078]">{new Date(item.submittedAt).toLocaleString("vi-VN")}</td></tr>) : <tr><td colSpan={6} className="px-5 py-10 text-center text-[#718078]">Chưa có bản ghi cục bộ. Hãy gửi một bài để bảng thống kê bắt đầu ghi nhận.</td></tr>}</tbody></table></div></div></section>
