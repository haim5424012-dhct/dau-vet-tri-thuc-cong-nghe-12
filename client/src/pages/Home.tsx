...
export default function Home() {
  ...
  const [form, setForm] = useState({ student: "", className: "", reportTitle: "", reportLink: "" });
  ...
  useEffect(() => {
    ...
    if (saved) {
      try { const parsed = JSON.parse(saved); setForm(parsed.form ?? form); setRubric(parsed.rubric ?? defaultRubric); } catch { /* ignore malformed local draft */ }
    }
  }, []);
  ...
  const submit = async () => {
    if (!form.student || !form.className || !form.reportTitle || !form.reportLink || answered < criteria.length) {
      toast.error("Vui lòng hoàn thiện thông tin nhóm, đường dẫn báo cáo và đủ 5 tiêu chí."); return;
    }
    ...
    if (!FORM_CONFIG.actionUrl) { exportDraft(); toast.info("Google Form chưa được cấu hình. Bản tóm tắt đã được lưu để không mất dữ liệu."); return; }
    try { await fetch(FORM_CONFIG.actionUrl, { method: "POST", mode: "no-cors", body }); setSubmitted(true); toast.success("Đã gửi kết quả vào đường ống Google Form."); }
    catch { toast.error("Không gửi được lúc này. Hãy tải bản tóm tắt dự phòng và thử lại."); }
  };
  ...
  return <div className="min-h-screen bg-[#f5f1e8] text-[#17352d]">
    ...
    <footer className="border-t border-[#d9d3c6] px-5 py-8 text-center text-xs text-[#7c8b83]">FIELDNOTE / CN12 · Công nghệ 12 Lâm nghiệp – Thủy sản · Dữ liệu tự chấm là phản tư tham khảo</footer>
  </div>;
}
